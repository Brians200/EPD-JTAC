...
PARSE_AVAILABLE_JTAC_ATTACKS = {
	...
	_numJtacAttacks = count EPDJtacAvailableAttacks;
	for "_jtackAttackI" from 0 to _numJtacAttacks -1 do {
		private ["_currentAttack", "_attackType"];
		_currentAttack = EPDJtacAvailableAttacks select _jtackAttackI;
	...
	for "_bulletsI" from 0 to _bulletsCount -1 do {
		...
		_innerExpressionString = format ["[['%1', %2, %3, '[",  _currentBullet select 0, _currentBullet select 2, _currentBullet select 3] +
								"%1, %2," +
