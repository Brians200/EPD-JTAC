...
PARSE_AVAILABLE_JTAC_ATTACKS = {
	private ["_numJtacAttacks", "_jtackAttackI", "_bullets", "_shells", "_bombs", "_rockets", "_mines", "_nonlethal", "_bulletsCount", "_keyNumber", "_guidedMissiles", "_strafingRun"];
	...
	for "_jtackAttackI" from 0 to _numJtacAttacks -1 do {
		...
		if(_attackType == "BULLETS") then {
			...
			_strafingRun set [ count _strafingRun, [_currentAttack select 1, _currentAttack select 2, _currentAttack select 3, _currentAttack select 4, _currentAttack select 5]];
